 ...
 *
 * BROWSER COMPATIBILITY: If negative lookbehind regex is not supported (older browsers),
 * the function falls back to only converting newlines to <br> tags without escaping < and >.
 *
 ...
 *
 * @param text - The text to sanitize. Can be null or undefined.
 * @returns The sanitized text safe for HTML rendering, or the original value if null/undefined.
 *
 * @example
 * sanitizeTextForRender('Hello\nWorld') // 'Hello<br>World'
 * sanitizeTextForRender('if x < 5') // 'if x &lt; 5' (or 'if x < 5<br>' on older browsers)
 ...
 */
// Feature detection for negative lookbehind support
function supportsNegativeLookbehind(): boolean {
  try {
    new RegExp('(?<!test)');
    return true;
  } catch {
...
export function sanitizeTextForRender(text: string | null | undefined) {
  ...
  // If negative lookbehind is not supported, return text with only newlines converted
